<?php
/**
 * Конфигурационный файл для загруженных файлов.
 * 
 * Данный файл необходимо подключить в bootstrap.php строкой Configure::load('files');
 * Получить данные конфигурационного файла можно при помощи строки, например: $load = Configure::read('load.scrBaseUrl');
 */
return [
	/**
	...
	'load' => [
		...
		// Изображения.
        'scrBaseUrl' => 'http://localhost/lovesafe.loc' . DS,// Домен или поддомен для фотографий.
        'scrUrl' => ROOT . DS . 'logs' . DS,// Место, где располагаются на сервере фотографии.
        'scrExt' => ['jpeg', 'bmp', 'gif', 'jp2', 'png', 'tiff'],// Расширения файлов, которые будем хранить в вышеуказанной директории.
	],
